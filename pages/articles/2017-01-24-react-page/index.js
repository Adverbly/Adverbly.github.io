        ...
        <div className="blog-single">
          <div className="text">
            ...
            <div id="content">
              ...
              <BlockComputationSingle associative={true} commutes={false} numTerms="3"/>
              ...
              <BlockComputationSingle associative={true} commutes={false} numTerms="5"/>
              ...
              <p>12 configurations. Why 12? The argument order can be arranged in n!(6 for n=3) different ways, and each
                ...
                right. In the general case, we get the nth <a href="https://en.wikipedia.org/wiki/Catalan_number"><b>Catalan
                  Number</b></a>. For anyone not already familiar with the Catalan Numbers, <i>which certainly would have included myself prior to this excursion,</i> these numbers show up commonly in combinatorics, but one application that caught my eye was for full binary trees. In fact, these abstracted computations that we have been drawing up to now have actually just been full binary trees, where the leaf nodes are arguments, and the parent nodes are the result of their children's evaluation.</p>
              <p>The leaf nodes represent the inputs, and the structure of the tree represents exactly how those arguments are consumed. Neat! Okay so let's bring this back to associativity and commutativity now. How does adding these properties change the relationship between the computation and that computation's representation?</p>
              <p>If we add both properties, we are guaranteed the same result as other computation with the same count of each argument. For any associative and commutative function,</p>

              <BlockComputationSingle associative={false} commutes={false} initComputation={[[4, 1], [3, 2],[[5, 2], [[1, 2], 3]]]}/>
              <p>is the same as</p>
              <BlockComputationSingle associative={false} commutes={false} initComputation={[[[[[[[[5,2],2],1],1],1],3],3],4]}/>
                <p>To verify this, all you have to do is count up each color across both examples. This means that the structure itself is completely meaningless. All we actually need to perform the evaluation is a count of the number of times that each argument has been included in the evaluation. A hash map where the key is the argument and the value is the number of times that argument is used, would be able to suit our needs for representing our computation.</p>
            </div>
          </div>
        </div>
      </div>
